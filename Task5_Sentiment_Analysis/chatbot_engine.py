...
SENTIMENT_WORDS = {
    "positive": {
        "amazing", "awesome", "best", "excellent", "fantastic", "fine", "good",
        "great", "happy", "helpful", "love", "nice", "perfect", "satisfied",
        "super", "thanks", "thank", "wonderful", "धन्यवाद", "अच्छा", "खुश",
        "बहुत", "शुक्रिया", "gracias", "bueno", "excelente", "feliz",
        "perfecto", "merci", "bon", "excellent", "parfait", "heureux",
    },
    "negative": {
        "angry", "awful", "bad", "broken", "complaint", "confused", "delay",
        "delayed", "disappointed", "error", "frustrated", "frustrating",
        "hate", "issue", "late", "mad", "problem", "refund", "sad", "terrible",
        "unhappy", "upset", "worst", "गलत", "गुस्सा", "दिक्कत", "नाराज",
        "परेशान", "समस्या", "malo", "problema", "enojado", "triste",
        "terrible", "frustrado", "mauvais", "probleme", "problème",
        "triste", "terrible", "frustre", "frustré",
    },
}
...
SENTIMENT_PHRASES = {
    "positive": {
        "thank you", "well done", "works great", "very helpful", "really helpful",
        "बहुत अच्छा", "muchas gracias", "très bien", "tres bien",
    },
    "negative": {
        "not working", "does not work", "did not work", "very bad", "too late",
        "worst service", "really frustrating", "so frustrating", "not happy",
        "बहुत खराब", "काम नहीं", "no funciona", "muy malo", "ne fonctionne pas",
    },
}


SENTIMENT_RESPONSE_PREFIX = {
    "en": {
        "positive": "I'm glad this is going well. ",
        "negative": "I'm sorry this has been frustrating. I'll keep this clear and help you fix it. ",
        "neutral": "",
    },
    "hi": {
        "positive": "यह सुनकर अच्छा लगा। ",
        "negative": "मुझे खेद है कि आपको परेशानी हुई। मैं शांत तरीके से आपकी मदद करता हूँ। ",
        "neutral": "",
    },
    "es": {
        "positive": "Me alegra que vaya bien. ",
        "negative": "Siento que esto haya sido frustrante. Te ayudaré paso a paso. ",
        "neutral": "",
    },
    "fr": {
        "positive": "Je suis content que cela se passe bien. ",
        "negative": "Je suis désolé que ce soit frustrant. Je vais vous aider clairement. ",
        "neutral": "",
    },
}
...
class MultilingualChatbot:
    ...
    def reply(self, message: str, session_id: str = "default") -> Dict[str, object]:
        ...
        sentiment, sentiment_score, sentiment_hits = self.detect_sentiment(message, tokens)
        ...
        fallback_response = self._apply_sentiment_tone(
            fallback_response,
            sentiment,
    ...
    def _openrouter_messages(
        ...
        system_prompt = (
            ...
            "adapt tone to the detected sentiment: celebrate positive users, use empathy "
            "and reassurance for negative users, and stay direct for neutral users. "
    ...
    def detect_sentiment(self, message: str, tokens: List[str]) -> Tuple[str, float, List[str]]:
        lowered = message.lower()
        token_set = set(tokens)
        positive_hits = sorted(token_set & SENTIMENT_WORDS["positive"])
        negative_hits = sorted(token_set & SENTIMENT_WORDS["negative"])

        for phrase in SENTIMENT_PHRASES["positive"]:
            if phrase in lowered:
                positive_hits.append(phrase)
        for phrase in SENTIMENT_PHRASES["negative"]:
            if phrase in lowered:
        ...
        if re.search(r"\b(no|not|never|n't)\s+\w+", lowered):
            negative_score += 1
            negative_hits.append("negation")
        if any(mark in message for mark in ("!", "!!")) and negative_score > positive_score:
            negative_score += 0.5
        if any(mark in message for mark in ("!", "!!")) and positive_score > negative_score:
            positive_score += 0.25

        total = positive_score + negative_score
        if total == 0 or abs(positive_score - negative_score) < 0.75:
            return "neutral", 0.0, []

        score = round((positive_score - negative_score) / max(total, 1), 2)
        if score >= 0.25:
            return "positive", score, positive_hits[:5]
        if score <= -0.25:
            return "negative", score, negative_hits[:5]
    ...
    def _apply_sentiment_tone(self, response: str, sentiment: str, lang: str) -> str:
        prefix = SENTIMENT_RESPONSE_PREFIX.get(lang, SENTIMENT_RESPONSE_PREFIX["en"]).get(sentiment, "")
        if not prefix or response.startswith(prefix):
            return response
