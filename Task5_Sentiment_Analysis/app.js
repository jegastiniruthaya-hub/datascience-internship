...
const sendMessage = async (message) => {
  ...
  sentiment.textContent = `${data.sentiment.label} (${data.sentiment.score})`;
  sentiment.dataset.value = data.sentiment.label;
  turns.textContent = data.context.turns;
  engine.textContent = data.engine;
  const evidence = data.sentiment.evidence.length
    ? ` · Evidence: ${data.sentiment.evidence.join(", ")}`
    : "";
  const detail = `Language: ${data.languages_in_message.join(", ")} · Intent confidence: ${data.confidence} · Sentiment: ${data.sentiment.label}${evidence}`;
...
addMessage(
  "bot",
  "Hello. I can detect whether a customer sounds positive, negative, or neutral, then adjust my response tone."
);
loadApiStatus();
